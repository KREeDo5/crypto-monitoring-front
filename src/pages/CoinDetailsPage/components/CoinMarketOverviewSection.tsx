...
export const CoinMarketOverviewSection: React.FC = () => {
  ...
  const lastLoadedPeriodRef = React.useRef<number | null>(null);
...
useEffect(() => {
  ...
  const loadHistory = async (showLoader: boolean) => {
  ...
  const shouldShowLoader = lastLoadedPeriodRef.current !== activePeriod;
  lastLoadedPeriodRef.current = activePeriod;

  void loadHistory(shouldShowLoader);
    ...
    <Paper elevation={0} sx={{ py: 2, background: "none" }}>
          ...
          <Stack spacing={2.5} sx={{ width: "100%" }}>
            ...
            <Stack spacing={1.25}>
              ...
              >
                ...
                >
                  {coinInfo.marketCap
                    ? formatCompactPrice(coinInfo.marketCap)
                    : "N/A"}
                </Typography>
              </Stack>
              ...
              >
                ...
                >
                  {coinInfo.volume24h
                    ? formatCompactPrice(coinInfo.volume24h)
                    : "N/A"}
                </Typography>
              </Stack>
              ...
              >
                ...
                <Typography
                  sx={{
                    ...
                    fontWeight: 500,
                    color:
                      coinInfo.hourChange >= 0 ? "success.main" : "error.main",
                  }}
                >
                  {coinInfo.hourChange != null
                    ? (coinInfo.hourChange > 0 ? "+" : "") +
                      coinInfo.hourChange.toFixed(2) +
                      "%"
                    : "N/A"}
                </Typography>
              </Stack>
              ...
              >
                ...
                <Typography
                  sx={{
                    ...
                    fontWeight: 500,
                    color:
                      coinInfo.dayChange >= 0 ? "success.main" : "error.main",
                  }}
                >
                  {coinInfo.dayChange != null
                    ? (coinInfo.dayChange > 0 ? "+" : "") +
                      coinInfo.dayChange.toFixed(2) +
                      "%"
                    : "N/A"}
                </Typography>
              </Stack>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  ...
                  >
                    ...
                    <XAxis
                      ...
                      tickFormatter={(index) =>
                        chartData[index]
                          ? formatXAxis(chartData[index].timestamp)
                          : ""
                      }
                      height={50}
                      interval={Math.ceil(Math.max(chartData.length, 1) / 6)}
                      minTickGap={20}
                      angle={-30}
                      textAnchor="end"
                      tick={{
      ...
      <Collapse in={isAiSummaryOpen} timeout={320}>
        ...
        >
          ...
          >
            *Ответ сгенерирован ИИ на основе текущих данных и не является
          ...
          {aiLoading ? (
            ...
            <Box
              sx={{ color: "text.secondary", fontSize: 16, lineHeight: 1.6 }}
            >
              <ReactMarkdown
